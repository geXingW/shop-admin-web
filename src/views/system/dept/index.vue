...
export default {
  ...
  methods: {
    getDeptDatas(tree, treeNode, resolve) {
      ...
      setTimeout(() => {
        crudDept.getDepts(params).then(({ data }) => {
    ...
    getDepts() {
      crudDept.getDepts({ enabled: true }).then(({ data }) => {
    ...
    loadDepts({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudDept.getDepts({ enabled: true, pid: parentNode.id }).then(({ data }) => {
          parentNode.children = data.records.map(function(obj) {
            if (obj.hasChildren) {
              obj.children = null
            }
            return obj
