...
export default {
  ...
  cruds() {
    return CRUD({ title: '角色', url: 'api/role', sort: 'level,asc', crudMethod: { ...crudRoles }})
  },
  ...
  methods: {
    ...
    menuChange(menu) {
      // 获取该节点的所有子节点，id 包含自身
      getChild(menu.id).then(({ data: childIds }) => {
    ...
    // 获取部门数据
    getDepts() {
      getDepts({ enabled: true }).then(({ data: res }) => {
    ...
    loadDepts({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        getDepts({ enabled: true, pid: parentNode.id }).then(({ data: res }) => {
