          ...
          <div v-for="(item, attributeId) in selectSaleAttributes" :key="attributeId">
            ...
            <div v-if="item && item.inputType == 0">
              ...
              <el-checkbox-group v-model="selectSaleAttributes[attributeId].values">
                <div v-for="(val, index) in attributeInputAddValues[attributeId].values" :key="index"
                     style="display: inline-block"
                >
                  <el-checkbox :label="val" class="littleMarginRight" @change="updateVueComponents"/>
                  <el-button type="text" class="largeMarginRight"
                             @click="handleRmAttributeInputValue(attributeId, index, val)"
                  >删除
                  </el-button>
                </div>
              </el-checkbox-group>
            ...
            <div v-if="item && item.inputType == 1">
              <span>{{ `${item.attributeName}：` }}</span>
              <el-checkbox-group v-model="selectSaleAttributes[attributeId].values">
                <el-checkbox :label="val" v-for="(val, index) in item.inputValue.split(',')" :key="index"
                             @change="updateVueComponents"
                />
              </el-checkbox-group>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    loadCategoryGroupAttribute(categoryId, isCreated) {
      ...
      groupAttributes(categoryId).then((res) => {
        if (res.status == 200000) {
          ...
          console.log(res.data.baseAttributes)
          console.log(this.value.attributeList)
        }
      })
    },
    ...
    doGenerateSkuList() {
      ...
      this.value.skuOptions = skuOptions
